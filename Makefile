...
check-git:
	...
	@if ! git remote -v | grep "upstream[[:space:]]git@github.com:ethereum/eth-keyfile.git (push)\|upstream[[:space:]]https://github.com/ethereum/eth-keyfile (push)"; then \
		echo "Error: You must have a remote named 'upstream' that points to 'eth-keyfile'"; \
		exit 1; \
	fi
